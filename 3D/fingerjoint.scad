...
*Three Dimensional Layout with Face Control*
- Show only front and top faces:
3Dlayout(showBack = false, showBottom = false, showLeft = false, showRight = false);
- Show only internal view (no top):
3Dlayout(showTop = false);
- Show only bottom and sides:
3Dlayout(showTop = false, showFront = false, showBack = false);
...
module 3Dlayout(xDim = 100, yDim = 100, zDim = 100, finger = 8, material = 5, text = true, 
                 showTop = true, showBottom = true, showFront = true, showBack = true, 
                 showLeft = true, showRight = true) {
  dim = [xDim, yDim, zDim];
  
  //bottom and top of box (XY faces)
  for( i=[-1,1]) {
    // Check if we should show this face
    showFace = (i == -1 && showBottom) || (i == 1 && showTop);
  ...
  //front and back of box (XZ faces)
  for (i=[-1,1]) {
    // Check if we should show this face
    showFace = (i == -1 && showBack) || (i == 1 && showFront);
  ...
  //left and right sides of box (YZ faces)
  for(i=[-1,1]) {
    // Check if we should show this face
    showFace = (i == -1 && showLeft) || (i == 1 && showRight);
...
//created by Aaron Ciuffo
  ...
  modules =
            [["outsideCut",
             ...
             ["3Dlayout",
              "module: 3Dlayout(xDim = <real>, yDim = <real>, zDim = <real>, finger = <real>, material = <real>, text = <boolean>, showTop = <boolean>, showBottom = <boolean>, showFront = <boolean>, showBack = <boolean>, showLeft = <boolean>, showRight = <boolean>)",
              "returns: none (module)",
              "Description: Demonstration module to create a 3D model of a laser-cuttable box with selective face rendering",
              "This module is meant to be used as a model reference",
              "Parameters: ",
              "     xDim        <real>      X dimension of box",
              "     yDim        <real>      Y dimension of box",
              ...
              "     material    <real>      thickness of material to be used",
              "     text        <boolean>   include debugging text for reference",
              "     showTop     <boolean>   render top face (default: true)",
              "     showBottom  <boolean>   render bottom face (default: true)",
              "     showFront   <boolean>   render front face (default: true)",
              "     showBack    <boolean>   render back face (default: true)",
              "     showLeft    <boolean>   render left face (default: true)",
              "     showRight   <boolean>   render right face (default: true)"
              ],
